  ...
  <div class="section">
    <div class="columns is-centered">
      <div class="column is-full-touch is-half-desktop">
        ...
        <div class="modal" :class="{ 'is-active': showedModal }">
        ...
        <div class="box">
          <table class="table is-fullwidth is-narrow is-hoverable">
            <thead>
              <tr>
                <th>{{ __('labels.name') }}</th>
                <th>{{ __('Available schedules') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr
                v-for="publisher in publishersFiltered"
                :key="publisher.id">
                <td class="has-text-vcentered">@{{ publisher.name }}</td>
                <td class="has-text-vcentered">
                  <div
                    ...
                    <div class="dropdown-trigger is-fullwidth">
                      <button
                        ...
                        @click.stop="showDropdown($event, publisher)"
                        aria-haspopup="true">
                        <template v-for="(schedule, index) in publisher.schedules[day]">
                          @{{ schedule }}
                          <br v-if="index < publisher.schedules[day].length - 1">
                        </template>
                      </button>
                    </div>
                    <div
                      ...
                      <div class="dropdown-content">
                        <a
                          v-for="(schedule, index) in publisher.schedules[day]"
                          :key="index"
                          @click="selectPublisher(publisher, schedule)"
                          class="dropdown-item">
                          @{{ schedule }}
                        </a>
                      </div>
